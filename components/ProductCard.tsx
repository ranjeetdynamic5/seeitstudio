    ...
    <div className="flex flex-col bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-[0_18px_50px_rgba(0,0,0,0.10)] hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <Link
            href={href}
            className="text-base font-semibold text-[#0B0F19] leading-snug hover:text-[#0066FF] transition-colors line-clamp-2"
          >
            {product.title}
          </Link>
            ...
            <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
              {(platform === 'windows' || platform === 'both') && (
                <div className="group/win relative">
                  <svg className="w-4 h-4 text-slate-400 hover:text-[#0066FF] transition-colors cursor-pointer" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M0 3.449L9.75 2.1v9.451H0m10.949-9.602L24 0v11.4H10.949M0 12.6h9.75v9.451L0 20.699M10.949 12.6H24V24l-12.9-1.801" />
                  </svg>
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-[#0B0F19] text-white text-[10px] font-medium px-2 py-1 rounded-md whitespace-nowrap opacity-0 group-hover/win:opacity-100 transition-opacity pointer-events-none z-50">
                    Windows
              ...
              {(platform === 'mac' || platform === 'both') && (
                <div className="group/mac relative">
                  <svg className="w-4 h-4 text-slate-400 hover:text-[#0B0F19] transition-colors cursor-pointer" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701" />
                  </svg>
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-[#0B0F19] text-white text-[10px] font-medium px-2 py-1 rounded-md whitespace-nowrap opacity-0 group-hover/mac:opacity-100 transition-opacity pointer-events-none z-50">
                    Mac
                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[#0B0F19]" />
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-3 border-t border-slate-100">
          <div className="flex flex-col">
            <span className="text-[10px] text-slate-400 uppercase tracking-wide">From</span>
            {product.is_on_sale && product.original_price != null ? (
              <div className="flex items-baseline gap-1.5">
                <span className="text-xl font-bold text-[#0B0F19]">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
              added
                ...
                : "bg-[#0B0F19] text-white hover:bg-[#0066FF]"
            }`}
          >
